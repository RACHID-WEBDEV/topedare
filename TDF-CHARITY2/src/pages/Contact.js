...
const Contact = () => {
    ...
    const { data, error } = useSWR(`${process.env.REACT_APP_BASE_URL}public/contact/unit/pull?user=${process.env.REACT_APP_USER_lOGIN_ID}`)
    ...
    const onSubmit = (data) => {
        const formOutput = { id: unique_id, ...data, unit: { value: messageUnit.email, label: messageUnit.name } }
        alert(JSON.stringify(formOutput));

        const result = JSON.stringify(formOutput)
