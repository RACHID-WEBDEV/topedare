import blogphoto from "../assets/hero/hero-banner-1-slide3.jpg"
import blogphoto1 from "../assets/hero/hero-banner-1-slide2.jpg"
import blogphoto2 from "../assets/hero/hero-banner-1.jpg"
import BlogImage from "../assets/blog/TDF-images.jpg"


const blogdata = [
    {
        blogImg: BlogImage,
        comments: "4",
        category: "Press Release",
        short_desc: "An charitable organization, TOPE DARE FOUNDATION has reduced financial burdens of some parents in Ondo state by paying the 2021 JAMB examination fees of all the 149 final year students of St. Peter’s Unity Secondary, Akure",
        description: <div>
            <p>An charitable organization, TOPE DARE FOUNDATION has reduced financial burdens of some parents in Ondo state by paying the 2021 JAMB examination fees of all the 149 final year students of St. Peter’s Unity Secondary, Akure.
            </p>
            <p>
                The founder of the organization, Engr Tope Dare, stated this in a post to one of the parents and obtained by SUNSHINETRUTH in Akure on Friday
            </p>
            <p>He said, “I am TOPE DARE an Old Student: In 1992, with the support of God and my late parents, I had an excellent WASC result with 7As and 2Cs and was opportuned to gain a university admission on 1st attempt of JAMB to study Electronic & Electrical Engineering at Obafemi Awolowo University, Ile Ife.

                “At my early age, I was brought up to understand that education shows us the importance of hard work and, at the same time, helps us to grow and develop in the society.

                “In giving back to the society and more importantly the school that built my solid educational foundation, my charitable organization, Tope Dare Foundation, has paid for the 2021 JAMB examination fees for all the 149 final year students of St. Peter’s Unity Secondary, Akure because I believe these children have a future bright with hope to be great and successful.</p>
            <p>
                “It is my expectation that this little seed being sown today shall germinate with fruits to produce successful scholars, global professionals, technocrats and business men that the society and school will be proud of.
                <q>
                    “God bless the parents, teachers and administrator of St. Peter’s Secondary School. Thank you. Tope Dare
                    www.topedare.org”

                </q>

                About Tope Dare Foundation…..

                The Tope Dare Foundation is a charity initiative of Tope Dare with the goal of giving back to the society and helping the less privileged and vulnerable in our communities to alleviate human suffering and create opportunities for a better life.

                The Tope Dare Foundation reaches out primarily to children who are vulnerable and may have otherwise been left to roam the streets with no hopes, children who were perhaps destined to live in poverty or become miscreants in our society.

                We strive to set these children on their way to a future incredibly different from that of the generations before them.

                We believe they have a future bright with hope if we assist with the basic education, health care and empowerment.

                www.topedare.org
            </p>
        </div>,
        date: "21 Sept, 2022",
        title: "Tope Dare Foundation’ Pays JAMB Fees Of All Final Year Students In Ondo School",
        url: "/blog-details"
    },
    {
        blogImg: BlogImage,
        comments: "4",

        category: "Press Release",
        short_desc: " The Tope Dare Foundation is a charity initiative of Tope Dare with the goal of giving back to the society and helping the less privileged and vulnerable, to alleviate human suffering and create opportunities for a better life.",
        description: <div>
            <p>The Tope Dare Foundation is a charity initiative of Tope Dare with the goal of giving back to the society and helping the less privileged and vulnerable, to alleviate human suffering and create opportunities for a better life.</p>
            <p>
                According to the information that was gathered, Tope Dare made this known through one of his social media handle where he ask that the information should be passed across to parents and guardians who have their children in the school.
            </p>

            <q>
                “I am TOPE DARE an Old Student: In 1992, with the support of God and my late parents, I had an excellent WASC result with 7As and 2Cs and was opportuned to gain a university admission on 1st attempt of JAMB to study Electronic & Electrical Engineering at Obafemi Awolowo University, Ile Ife. At my early age, I was brought up to understand that education shows us the importance of hard work and, at the same time, helps us to grow and develop in the society.”
            </q>

            <p>
                “In giving back to the society and more importantly the school that built my solid educational foundation, my charitable organization, Tope Dare Foundation, has paid for the 2021 JAMB examination fees for all the 149 final year students of St. Peter’s Unity Secondary, Akure because I believe these children have a future bright with hope to be great and successful.”
            </p>
            <q>
                “It is my expectation that this little seed being sown today shall germinate with fruits to produce successful scholars, global professionals, technocrats and business men that the society and school will be proud of.”

            </q>
            <p>
                “God bless the parents, teachers and administrator of St. Peter’s Secondary School.” Tope Dare added.
            </p>



        </div>,
        date: "21 Sept, 2022",
        title: "Tope Dare Foundation Clears Jamb Exam Fees For 149 Final Year Students Of Unity School",
        url: "/blog-details"
    }, {
        blogImg: blogphoto2,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Donation is Hope for Poor Childrens in Africa 3",
        url: "/blog-details"
    }, {
        blogImg: blogphoto,
        comments: "4",
        category: "News",
        short_desc: "passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Donation is Hope for Poor Childrens in Africa 4",
        url: "/blog-details"
    }, {
        blogImg: blogphoto1,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Donation is Hope for Poor Childrens in Africa 5",
        url: "/blog-details"
    }, {
        blogImg: blogphoto2,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Donation is Hope for Poor Childrens in Africa not 6 ",
        url: "/blog-details"
    }, {
        blogImg: blogphoto,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Donation is Hope for Poor Childrens in Africa 7",
        url: "/blog-details"
    }, {
        blogImg: blogphoto1,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Donation is Hope for Poor Childrens in Africa 8",
        url: "/blog-details"
    }, {
        blogImg: blogphoto2,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Donation is Hope for Poor Childrens in Africa 9",
        url: "/blog-details"
    },
    {
        blogImg: blogphoto,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: "Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Donation is Hope for Poor Childrens in Africa 10",
        url: "/blog-details"
    },
    {
        blogImg: blogphoto1,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Donation is Hope for Poor Childrens in Africa 11",
        url: "/blog-details"
    }, {
        blogImg: blogphoto2,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: " but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Donation is Hope for Poor Childrens in Africa 12",
        url: "/blog-details"
    }, {
        blogImg: blogphoto,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: "of passages of Lorem Ipsum available, but the majority have humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Help with Donation is Hope for Poor Childrens in Africa 13",
        url: "/blog-details"
    }, {
        blogImg: blogphoto1,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: " in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Donation is Hope for Poor Childrens in Africa 14",
        url: "/blog-details"
    }, {
        blogImg: blogphoto2,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Help with  Donation is Hope for Poor Childrens in Africa 15",
        url: "/blog-details"
    }, {
        blogImg: blogphoto1,
        comments: "4",
        category: "News",
        short_desc: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour,",
        description: "of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use",
        date: "21 Sept, 2022",
        title: "Donation is Hope for Poor Childrens in Africa 16",
        url: "/blog-details"
    },
]
export default blogdata