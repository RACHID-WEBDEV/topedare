...
export const LargeBlogcard = ({ blogPhoto, category, short_desc, title, slug }) => (
    <div className="lg:w-6/12 group">
        ...
        <Link to={slug} className="underline">Read more</Link>
    </div>
)

export const SmallBlogcard = ({ blogPhoto, category, short_desc, title, slug }) => (
    ...
    return (
        <div className="my-14 bg-tdf-bg py-8 lg:px-10 lg:py-16">
            ...
            <div className="p-4 lg:p-12 flex flex-col lg:flex-row lg:space-x-20 ">
                {
                    blogdata.slice(0, 1).map(({ blogImg, category, short_desc, title, url }, idx) => (
                        <LargeBlogcard key={idx} blogPhoto={blogImg} category={category} slug={url}
                ...
                <div className="lg:w-7/12 flex flex-col justify-between mt-12 space-y-5 lg:space-y-0 lg:mt-0">
                    {
                        blogdata.slice(1, 4).map(({ blogImg, category, short_desc, title, url }, idx) => (
                            <SmallBlogcard key={idx} blogPhoto={blogImg} category={category} slug={url}
                                short_desc={short_desc} title={title} />
                        ))
                    }
